...
tags:
  ...
  - ClaudeCode
private: false
updated_at: '2026-03-23T13:08:41+09:00'
...
```mermaid
flowchart LR
    A[Claude Code for VS Code<br>コード生成] --> B[Databricks CLI<br>デプロイ]
...
3. コーディングエージェント連携の**始めましょう(Get Started)** をクリック
4. **その他の統合(Other Integrations)** タブ → **Claude Code**を選択
5. **デフォルトAnthropicモデル(Default Anthropic Model)** で`databricks-claude-sonnet-4-5`を選択
...
![スクリーンショット 2026-03-23 110753.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1168882/92d88ffd-2f81-4195-b448-185095ee3ca6.png)
...
VS Code上に**差分ビュー**が表示されます。これがVS Code拡張の最大の特徴です。

- 左側(変更前): 空
- 右側(変更後): Claude Codeが生成したコード(緑色でハイライト)
- 上部に**Accept**と**Reject**のボタン

内容を確認して**Accept**をクリックしてください。VS Codeのエクスプローラーに`hello.py`が追加されます。
...
今度は**既存ファイルの変更**の差分ビューが表示されます。赤色(削除される行)と緑色(追加される行)が並んで表示されるので、何がどう変わるかを確認してからAcceptしてください。
...
ここで**巻き戻し**を体験します。Step 2-1のメッセージ(hello.pyを最初に生成した時点)にマウスをホバーすると、巻き戻しボタンが表示されます。
...
| `requirements.txt` | Pythonの依存パッケージ | `streamlit`と`databricks-sdk`が含まれているか |
...
`valueFrom: sql-warehouse-id`は、デプロイ時にDatabricks側で実際のWarehouse IDに解決されます(Phase 5で設定します)。
...
Claude Codeが`app.py`の差分を提示します。Phase 2で練習した通り、VS Code上で変更内容を確認し、Acceptしてください。
...
:::note info
VS Codeで`uc-browser`フォルダを開いていれば、統合ターミナルのカレントディレクトリは自動的にそのフォルダになります。別のターミナルを使う場合は先に`cd %USERPROFILE%\uc-browser`を実行してください。
:::
...
4. **編集** をクリック
![スクリーンショット 2026-03-23 103252.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1168882/aa8122ea-3c90-44b4-9c62-97c71aa38067.png)
5. **アプリのリソース** セクションで **SQLウェアハウス** を追加
6. 使用するSQLウェアハウスを選択
![スクリーンショット 2026-03-23 103337.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1168882/90c28607-18d1-4641-8d0a-7d7255374575.png)
...
![スクリーンショット 2026-03-23 114642.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1168882/86dfbca1-762d-4a8e-a4b1-c4333e39c343.png)


`status`が`RUNNING`になれば成功です。デプロイ待ちの間に、ここまでの操作を振り返ったり、講師に質問したりしてください。
...
`databricks apps get`の出力に表示される**URL**をブラウザで開きます。

Unity Catalogテーブルブラウザが表示され、カタログ → スキーマ → テーブルを選択してデータプレビューが見られれば完成です!

![スクリーンショット 2026-03-23 110337.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/1168882/f1aaea00-6a61-4129-af54-86e033989c7d.png)
...
Phase 3でClaude Codeの生成がうまくいかない場合は、以下の内容でファイルを作成してください。VS Codeで`Ctrl+N` → 内容を貼り付け → `Ctrl+S`でファイル名を指定して保存します。
